...
class CommentAddViewModel @Inject constructor(
    ...
    private fun searchCategoryMain(Id: Int) {
        ...
        launch {
            when (val result = commentRepositoryImpl.getCommentableProduct(customSharedPreferences.getUserId()!!, Id)) {
                is Resource.Success -> {
    ...
    fun addComment(openDialog: MutableState<Boolean>) {
        isLoading.value = true
        launch {
            when (val result = commentRepositoryImpl.addComment(customSharedPreferences.getUserId()!!, list.value.productId, text.value, point.value)) {
                is Resource.Success -> {
                    errorMessage.value = ""
